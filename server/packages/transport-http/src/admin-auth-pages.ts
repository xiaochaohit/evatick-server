...
export const adminPasswordHtml = document('修改密码', `<main class="card"><div class="brand"><span class="mark">MC</span><span>Market Console</span></div><div class="eyebrow">ACCOUNT SECURITY</div><h1>修改密码</h1><p class="subtitle">新密码至少 12 个字符。修改成功后，其他已登录会话将立即失效。</p><form id="form"><label for="current">当前密码</label><input id="current" type="password" autocomplete="current-password" required><label for="next">新密码</label><input id="next" type="password" minlength="12" maxlength="256" autocomplete="new-password" required><label for="confirm">确认新密码</label><input id="confirm" type="password" minlength="12" maxlength="256" autocomplete="new-password" required><button id="submit" type="submit">保存新密码</button><div class="error" id="message" role="alert"></div></form><a class="back" href="/admin">← 返回管理后台</a></main>`, `
  const form=document.getElementById('form'),button=document.getElementById('submit'),message=document.getElementById('message');
