...
Market CLI 与 Market Server 在首版只通过版本化 HTTP API 通信。普通查询使用 JSON 响应信封；标的目录和指数成分等集合使用游标分页；较大结果后续通过 HTTP 压缩、NDJSON 流或异步 Parquet 导出扩展。CLI 默认只向调用者输出响应信封中的 `data`，保留现有成功结果直接输出数据本体的契约。
