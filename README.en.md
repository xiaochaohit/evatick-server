...
**A self-hosted China market data service for the EVA CLI and automated clients.**
...
EVA Tick Server (`evatickd`) presents market data from different providers through a versioned HTTP API. It maintains a canonical instrument catalog, performs search and resolution, routes requests with timeouts and fallback, normalizes results, and includes a local administration console for data operations.
