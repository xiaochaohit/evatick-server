...
function document(title: string, content: string, script: string): string {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} · EVA Tick Server</title><style>${pageStyles}</style></head><body>${content}<script>${script}</script></body></html>`
}
