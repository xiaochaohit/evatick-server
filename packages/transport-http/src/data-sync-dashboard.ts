...
<body>
  ...
  <main class="shell">
    ...
    <section class="grid">
      <aside class="panel form">
        ...
        <div class="field"><label for="interval">行情周期</label><select id="interval"><option value="1m" selected>1 分钟 · 三个月验证</option><option value="1d">日线</option></select></div>
  ...
  <script>
    const byId=(id)=>document.getElementById(id);const today=new Date();byId('end').value=today.toISOString().slice(0,10);const threeMonthsAgo=new Date(today);threeMonthsAgo.setUTCMonth(threeMonthsAgo.getUTCMonth()-3);byId('start').value=threeMonthsAgo.toISOString().slice(0,10);let scheduleLoaded=false;
    ...
    byId('start-sync').addEventListener('click',async()=>{const types=[];if(byId('equity').checked)types.push('equity');if(byId('index').checked)types.push('index');const limit=byId('limit').value;try{await request('/v1/data-sync/runs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({instrument_types:types,interval:byId('interval').value,start:byId('start').value,end:byId('end').value,adjustment:byId('adjustment').value,delay_ms:Number(byId('delay').value),...(limit==='all'?{}:{limit:Number(limit)})})});await refresh();}catch(error){byId('error').textContent=error.message;}});
