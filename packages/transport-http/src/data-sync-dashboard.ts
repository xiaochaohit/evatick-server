...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .schedule-grid { display:grid; grid-template-columns:1.2fr .8fr .8fr 1fr 1fr 1fr 1fr; gap:10px; align-items:end; }
...
<body>
  ...
  <script>
    const byId=(id)=>document.getElementById(id);const today=new Date();byId('end').value=today.toISOString().slice(0,10);const threeMonthsAgo=new Date(today);threeMonthsAgo.setUTCMonth(threeMonthsAgo.getUTCMonth()-3);byId('start').value=threeMonthsAgo.toISOString().slice(0,10);let scheduleLoaded=false;let startSubmitting=false;
