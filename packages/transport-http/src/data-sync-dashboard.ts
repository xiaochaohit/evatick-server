...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .scheduled-head { display:flex; align-items:center; justify-content:space-between; gap:16px; margin-bottom:12px; }
    .scheduled-head h3 { margin:0; font-size:14px; }
    .task-list { border:1px solid var(--line); border-radius:15px; overflow:hidden; background:#10201b; }
    .task-row { display:grid; grid-template-columns:.8fr 1.2fr .8fr .8fr 1.4fr auto; align-items:center; gap:14px; padding:14px 16px; }
    .task-row + .task-row { border-top:1px solid var(--line); }
    .task-label { display:none; color:var(--muted); font-size:10px; margin-bottom:4px; }
    .task-value { font-size:12px; }
    .task-interval { display:inline-flex; padding:5px 8px; border-radius:7px; color:var(--green); background:var(--mint); font-weight:700; }
    .revoke { padding:8px 11px; color:var(--red); background:rgba(190,73,55,.14); }
    .task-empty { padding:22px; color:var(--muted); font-size:12px; text-align:center; }
    @keyframes rise { from { opacity:0; transform:translateY(12px); } to { opacity:1; transform:none; } }
    @media(max-width:820px) { .grid{grid-template-columns:1fr}.metrics{grid-template-columns:1fr 1fr}header{align-items:flex-start;flex-direction:column}.schedule-grid{grid-template-columns:1fr 1fr}.schedule-save{width:100%}.task-row{grid-template-columns:1fr 1fr 1fr}.task-label{display:block}.revoke{width:100%} }
...
<body>
  ...
  <main class="shell">
    <header>
      <div><div class="eyebrow">LOCAL MARKET DATA PIPELINE</div><h1>历史数据同步</h1><p class="subtitle">将股票与指数的日线或 1 分钟行情柱写入本地 DuckDB。任务按标的串行执行，支持失败隔离和增量回补。</p></div>
    ...
    <section class="grid">
      <aside class="panel form">
        ...
        <div class="field"><label for="interval">行情周期</label><select id="interval"><option value="1m" selected>1 分钟</option><option value="1d">日线</option></select></div>
        ...
        <div class="note">本地仓库仅保存原始行情；日线股票同步会同时刷新复权因子，查询时动态生成前复权或后复权价格。</div>
        ...
        <button class="primary" id="start-sync">开始同步</button><button class="resume" id="retry-failures" disabled>仅重试失败标的</button><button class="resume" id="resume-sync" disabled>继续上次任务</button><button class="cancel" id="cancel-sync" disabled>取消任务</button><div class="error" id="error"></div>
      </aside>
      <article class="panel content">
        <div class="metrics"><div class="metric"><div class="metric-label">库内标的</div><div class="metric-value" id="instruments">—</div></div><div class="metric"><div class="metric-label">日线 / 1m</div><div class="metric-value" id="bars">—</div></div><div class="metric"><div class="metric-label">成功标的</div><div class="metric-value" id="succeeded">—</div></div><div class="metric"><div class="metric-label">失败标的</div><div class="metric-value" id="failed">—</div></div></div>
    ...
    <section class="panel schedule-panel">
      <div class="schedule-head"><div><h2>每日定时同步</h2><div class="note">日线和 1 分钟可以分别创建任务。按北京时间执行，已有标的回溯最近若干天；新标的自动建立完整历史。</div></div><div class="schedule-state" id="schedule-state">正在读取任务</div></div>
      <div class="schedule-grid">
        <div class="field"><label>标的分类</label><div class="checks"><label class="check"><input id="schedule-equity" type="checkbox" checked>股票</label><label class="check"><input id="schedule-index" type="checkbox" checked>指数</label></div></div>
        <div class="field"><label for="schedule-interval">行情周期</label><select id="schedule-interval"><option value="1d">日线</option><option value="1m">1 分钟</option></select></div>
      ...
      <button class="primary schedule-save" id="save-schedule">添加定时任务</button><div class="error" id="schedule-error"></div>
      <div class="scheduled"><div class="scheduled-head"><h3>当前定时任务</h3><span class="note" id="schedule-count">—</span></div><div class="task-list" id="schedule-list"><div class="task-empty">正在读取任务…</div></div></div>
    </section>
  </main>
  <script>
    const byId=(id)=>document.getElementById(id);const today=new Date();byId('end').value=today.toISOString().slice(0,10);const threeMonthsAgo=new Date(today);threeMonthsAgo.setUTCMonth(threeMonthsAgo.getUTCMonth()-3);byId('start').value=threeMonthsAgo.toISOString().slice(0,10);let startSubmitting=false;
    const esc=(v)=>String(v).replace(/[&<>'"]/g,(c)=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
    const statusName={running:'同步中',completed:'已完成',completed_with_errors:'完成 · 有异常',cancelled:'已取消',failed:'失败'};
    async function request(url,options){const response=await fetch(url,options);const body=response.status===204?null:await response.json();if(!response.ok)throw new Error(body?.detail||'请求失败');return body;}
    ...
    function renderSchedules(schedules){byId('schedule-count').textContent=schedules.length+' 个待执行任务';byId('schedule-state').textContent=schedules.length?'调度器运行中':'暂无定时任务';byId('schedule-list').innerHTML=schedules.length?schedules.map((schedule)=>'<div class="task-row"><div><span class="task-label">周期</span><span class="task-interval">'+(schedule.interval==='1m'?'1 分钟':'日线')+'</span></div><div><span class="task-label">标的分类</span><div class="task-value">'+schedule.instrument_types.map((type)=>type==='equity'?'股票':'指数').join('、')+'</div></div><div><span class="task-label">每日时间</span><div class="task-value mono">'+esc(schedule.time)+'</div></div><div><span class="task-label">回溯范围</span><div class="task-value">最近 '+schedule.lookback_days+' 天</div></div><div><span class="task-label">下次执行</span><div class="task-value mono">'+esc(formatTime(schedule.next_run_at))+'</div></div><button class="revoke" data-schedule-id="'+schedule.schedule_id+'">撤销</button></div>').join(''):'<div class="task-empty">暂无定时任务，可以在上方添加日线或 1 分钟任务。</div>';}
    function render(body){const data=body.data;const active=data.active_run;const run=active||data.last_run;const resumable=!active&&run&&['cancelled','failed'].includes(run.status)&&run.completed<run.total;const retryable=!active&&run&&run.failed>0&&((run.failed_instrument_ids?.length||run.errors?.some((error)=>error.instrument_id)));const startButton=byId('start-sync');byId('instruments').textContent=data.storage.instruments.toLocaleString();byId('bars').textContent=data.storage.daily_bars.toLocaleString()+' / '+data.storage.minute_bars.toLocaleString();byId('database').textContent=data.database_path;const dailyCoverage=data.storage.first_trading_date?(data.storage.first_trading_date+' → '+data.storage.last_trading_date):'暂无';const minuteCoverage=data.storage.first_minute_trading_date?(data.storage.first_minute_trading_date+' → '+data.storage.last_minute_trading_date):'暂无';byId('coverage').textContent='日线 '+dailyCoverage+'；1m '+minuteCoverage;startButton.disabled=startSubmitting||!!active;startButton.textContent=startSubmitting?'正在提交…':active?'同步中':'开始同步';startButton.setAttribute('aria-busy',String(startSubmitting));byId('retry-failures').disabled=!retryable;byId('resume-sync').disabled=!resumable;byId('cancel-sync').disabled=!active;renderRun(run,!!active);if(startSubmitting)byId('state').textContent='正在创建同步任务';else if(!active)byId('state').textContent=run?(statusName[run.status]||run.status):'待同步';renderSchedules(data.schedules);}
    ...
    byId('start-sync').addEventListener('click',async()=>{const button=byId('start-sync');const types=[];if(byId('equity').checked)types.push('equity');if(byId('index').checked)types.push('index');const limit=byId('limit').value;startSubmitting=true;button.disabled=true;button.textContent='正在提交…';button.setAttribute('aria-busy','true');byId('state').textContent='正在创建同步任务';byId('error').textContent='';try{await request('/v1/data-sync/runs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({instrument_types:types,interval:byId('interval').value,start:byId('start').value,end:byId('end').value,adjustment:'none',delay_ms:Number(byId('delay').value),...(limit==='all'?{}:{limit:Number(limit)})})});startSubmitting=false;button.textContent='同步中';await refresh();}catch(error){startSubmitting=false;button.disabled=false;button.textContent='开始同步';button.setAttribute('aria-busy','false');byId('state').textContent='任务创建失败';byId('error').textContent=error.message;}});
    ...
    byId('retry-failures').addEventListener('click',async()=>{try{await request('/v1/data-sync/retry-failures',{method:'POST'});await refresh();}catch(error){byId('error').textContent=error.message;}});
    ...
    byId('save-schedule').addEventListener('click',async()=>{const button=byId('save-schedule');const types=[];if(byId('schedule-equity').checked)types.push('equity');if(byId('schedule-index').checked)types.push('index');button.disabled=true;button.textContent='正在添加…';byId('schedule-error').textContent='';try{await request('/v1/data-sync/schedules',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({interval:byId('schedule-interval').value,time:byId('schedule-time').value,skip_weekends:byId('schedule-skip-weekends').checked,instrument_types:types,lookback_days:Number(byId('schedule-lookback').value),adjustment:'none',delay_ms:Number(byId('schedule-delay').value)})});byId('schedule-error').textContent='定时任务已添加';await refresh();}catch(error){byId('schedule-error').textContent=error.message;}finally{button.disabled=false;button.textContent='添加定时任务';}});
    byId('schedule-list').addEventListener('click',async(event)=>{const button=event.target.closest('[data-schedule-id]');if(!button)return;const scheduleId=button.dataset.scheduleId;if(!confirm('确定撤销这个定时任务吗？'))return;button.disabled=true;button.textContent='正在撤销…';try{await request('/v1/data-sync/schedules/'+scheduleId,{method:'DELETE'});byId('schedule-error').textContent='定时任务已撤销';await refresh();}catch(error){button.disabled=false;button.textContent='撤销';byId('schedule-error').textContent=error.message;}});
