...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .chart-wrap canvas { display:block; width:100%; height:420px; cursor:crosshair; }
...
<body>
  ...
  <main class="shell">
    ...
    <section class="panel">
      <div class="toolbar"><div class="search"><input id="query" type="search" placeholder="输入代码或名称，例如 000001、IF2609、BTC-USDT" autocomplete="off"></div><div class="filters"><button class="filter active" data-type="">全部</button><button class="filter" data-type="equity">股票</button><button class="filter" data-type="index">指数</button><button class="filter" data-type="future">期货</button><button class="filter" data-type="crypto">加密货币</button></div><div class="filters"><button class="period-filter active" data-interval="">全部周期</button><button class="period-filter" data-interval="1d">日线</button><button class="period-filter" data-interval="1m">1分钟</button></div></div>
  ...
  <script>
    ...
    const typeName=(type)=>type==='equity'?'股票':type==='index'?'指数':type==='future'?'期货':type==='crypto'?'加密货币':type;
    function instrumentTable(items){if(!items.length)return '<div class="empty">没有找到已落库的标的</div>';return '<table><thead><tr><th>标的</th><th>类型</th><th>交易场所</th><th>日线记录</th><th>1m 记录</th><th>1m 实际范围</th></tr></thead><tbody>'+items.map((item)=>'<tr data-id="'+esc(item.instrument_id)+'" data-label="'+esc(item.name+' · '+item.symbol+' · '+(item.venue||item.publisher||''))+'" data-minute="'+item.minute_records+'" data-minute-end="'+esc(item.minute_last_trading_date||'')+'" data-daily-end="'+esc(item.daily_last_trading_date||'')+'"><td class="identity"><strong>'+esc(item.name)+'</strong><span class="mono">'+esc(item.symbol)+'</span></td><td><span class="type">'+esc(typeName(item.instrument_type))+'</span></td><td class="mono">'+esc(item.venue||item.publisher||'—')+'</td><td class="number">'+formatNumber(item.daily_records)+'</td><td class="number">'+formatNumber(item.minute_records)+'</td><td class="mono hide-mobile">'+range(item.minute_first_trading_date,item.minute_last_trading_date)+'</td></tr>').join('')+'</tbody></table>';}
    ...
    function drawChart(){
      const canvas=byId('bar-chart-canvas'),wrap=byId('bar-chart'),empty=byId('bar-chart-empty'),rows=state.barRows;
    ...
    function setBarRows(rows,message){state.barRows=rows.slice().reverse();state.hoveredBar=-1;byId('bar-chart-empty').textContent=message||'所选范围内没有本地记录';byId('bar-chart-tooltip').hidden=true;scheduleChart();}
    ...
    async function loadBars(){if(!state.selectedId)return;if(barsController)barsController.abort();barsController=new AbortController();const signal=barsController.signal;const params=new URLSearchParams({interval:state.detailInterval,limit:String(state.barLimit),offset:String(state.barOffset)});if(state.detailDate){params.set('start',state.detailDate);params.set('end',state.detailDate);}byId('bars').innerHTML='<div class="empty">正在读取本地行情…</div>';setBarRows([],'正在读取本地行情…');try{const body=await request('/v1/local-data/instruments/'+encodeURIComponent(state.selectedId)+'/bars?'+params,{signal});state.barTotal=body.page.total;byId('bars').innerHTML=barTable(body.data);setBarRows(body.data);const from=state.barTotal?state.barOffset+1:0;const to=Math.min(state.barOffset+state.barLimit,state.barTotal);byId('bar-summary').textContent='本地记录 '+from+'–'+to+' / '+formatNumber(state.barTotal);byId('bar-previous').disabled=state.barOffset===0;byId('bar-next').disabled=state.barOffset+state.barLimit>=state.barTotal;}catch(error){if(error.name==='AbortError')return;byId('bars').innerHTML='<div class="empty">'+esc(error.message)+'</div>';setBarRows([],error.message);}}
    ...
    byId('bar-chart-canvas').addEventListener('pointermove',(event)=>{if(!state.barRows.length)return;const canvas=event.currentTarget,rect=canvas.getBoundingClientRect(),left=58,right=16,plotWidth=Math.max(1,rect.width-left-right),relative=Math.max(0,Math.min(plotWidth-1,event.clientX-rect.left-left)),index=Math.max(0,Math.min(state.barRows.length-1,Math.floor(relative/plotWidth*state.barRows.length))),bar=state.barRows[index],tooltip=byId('bar-chart-tooltip');state.hoveredBar=index;tooltip.textContent=chartTime(bar)+'  开 '+bar.open+'  高 '+bar.high+'  低 '+bar.low+'  收 '+bar.close+'  量 '+shortNumber(bar.volume);tooltip.hidden=false;const tooltipWidth=tooltip.offsetWidth,tooltipHeight=tooltip.offsetHeight;tooltip.style.left=Math.max(4,Math.min(rect.width-tooltipWidth-16,event.clientX-rect.left))+'px';tooltip.style.top=Math.max(4,Math.min(rect.height-tooltipHeight-16,event.clientY-rect.top))+'px';scheduleChart();});
    byId('bar-chart-canvas').addEventListener('pointerleave',()=>{state.hoveredBar=-1;byId('bar-chart-tooltip').hidden=true;scheduleChart();});
