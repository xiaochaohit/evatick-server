...
<body>
  ...
  <script>
    ...
    byId('bar-chart').addEventListener('wheel',(event)=>{event.preventDefault();event.stopPropagation();if(!state.barRows.length)return;const rect=byId('bar-chart-canvas').getBoundingClientRect(),anchor=Math.max(0,Math.min(1,(event.clientX-rect.left-58)/(rect.width-58-16)));if(Math.abs(event.deltaX)>Math.abs(event.deltaY))panChart(event.deltaX/12);else zoomChart(Math.exp(event.deltaY*.0015),anchor);},{passive:false});
